...
export function renderSlideContent(slide, assetResolver) {
    ...
    const h5pContent = sanitizedHtml.replace(
        /<p>\s*@@kicampus_h5p:([^@]+)@@\s*<\/p>/g,
        (_, pluginFilePath) => {

            const encodedPluginFileUrl = encodeURIComponent(
                `https://moodle.ki-campus.org/pluginfile.php/${pluginFilePath}`
            );

            const iframeUrl =
                `https://moodle.ki-campus.org/h5p/embed.php?url=${encodedPluginFileUrl}`;
